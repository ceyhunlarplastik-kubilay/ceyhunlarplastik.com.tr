# Improvement Log (Arşiv)

> **Bu dosya kronolojik arşivdir — projenin hafızası.** 2026-08-29'da
> [IMPROVEMENT_PLAN.md](IMPROVEMENT_PLAN.md) ikiye ayrıldı: PLAN yalnızca **açık
> (henüz yapılmamış) işleri** tutar, tamamlanan dilimlerin tarihli uygulama notları
> buraya taşındı. Bir dilim bitince maddesi PLAN'dan çıkar, notu buraya eklenir.
> Aşağıdaki içerik ayrımdan önceki hâliyle korunmuştur (2026-07-07 denetiminden
> 2026-08-28'e kadar); "Öncelikli Aksiyon Listesi"ndeki ✅ işaretli maddeler
> tamamlanmıştır, açık kalanların güncel takibi PLAN'dadır.
...
**P2.6 — SST 4 yükseltme kararı** · kapsam: büyük, ayrı proje · **✅ TAMAMLANMIŞ (2026-08-25 tespiti)** — repo `sst@4.17.1` kullanıyor; bu madde bayattı. P2.7'nin bloğu da bununla kalkıyor.
...
üzerinden `upsertProductVariantRows`'a gidiyor; tedarikçi ise talep formunda tüm
renk/hammadde kataloğundan seçiyor ve ürünün versiyon sözlüğünü GÖRMÜYOR. Tanımsız
kombinasyon **onay anında** reddediliyor.

Bozulma yok: transaction geri alınıyor, talep duruyor, onaylayan versiyonu tanımlayıp
yeniden onaylayabiliyor. Eksik olan yalnız hatanın ne zaman görüldüğü — onaylayan
düğmeye bastıktan sonra öğreniyor ve mesaj İngilizce.

**Karar: A — şimdilik bırakıldı** (kullanıcı, 2026-08-25). Asıl veri girişi yolu
...
### Düzeltme — nokta seçilince arama kutusu boş görünüyordu (2026-08-28, aynı gün)
- **Bildirilen sorun:** Kullanıcı, başlangıç/bitiş noktasını (arama, "mevcut
  konumum" veya haritadan seçim — üçü de aynı şekilde) belirledikten sonra
  [RouteEndpointPicker.tsx](packages/frontend/features/customerLocations/routePlanning/RouteEndpointPicker.tsx)'daki
  arama kutusunun boş göründüğünü bildirdi.
- **Kök neden:** `handlePlaceSelect` seçimden sonra `clearSearch()` çağırıyordu
  ve bu `searchInput` state'ini de boşaltıyordu. Seçilen noktanın adı yalnızca
  kutunun ÜSTÜNDEKİ küçük özet satırında (`point?.label || "Seçilmedi"`)
  gösteriliyordu — arama kutusunun kendisi hiçbir zaman seçilen noktayı
  yansıtmıyordu; ayrıca "mevcut konumum" ve haritadan seçim yollarında da
  arama kutusu hiç güncellenmiyordu.
- **Düzeltme:** Arama kutusu artık `point` prop'u değişince (üç seçim yolu da
  sonunda bunu değiştiriyor) seçilen noktanın adını gösteriyor. Bunu
  `useEffect`+`setState` ile YAPMADIM — repo'nun eslint kuralı efekt içinde
  senkron `setState`'i hata sayıyor (aşağıdaki `SidebarShortcutKbd` notuyla
  aynı ders).
  Bunun yerine React'ın "prop değişince state'i uyarla" deseni kullanıldı:
  render sırasında `point?.refId`'i önceki senkronize edilen id ile
  karşılaştırıp farklıysa state'i doğrudan güncelliyor (bkz.
  `syncedPointRefId`). Aramayı temizle (X) butonunun görünürlüğü de
  `submittedQuery` yerine `searchInput`'a bağlandı — aksi halde seçim sonrası
  kutuda isim görünürken X butonu kayboluyordu.
- **Not:** Kullanıcı arama kutusuna elle yazmaya devam ederse bu yeni bir
  arama başlatır ve mevcut seçim değişene kadar kutu kullanıcının yazdığını
  gösterir — beklenen davranış.
- Doğrulama: `typecheck -w frontend` ✅ `lint -w frontend` 0 error (157 mevcut
  warning değişmedi) ✅ `test -w frontend` 336/336 ✅ (sayı `codex/cart-load-visualizer`
  merge'inin eklediği testlerle 324'ten 336'ya çıktı, bu değişiklikle ilgisiz).
