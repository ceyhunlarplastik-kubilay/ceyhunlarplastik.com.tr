...
export const updateProductHandler = ({ productRepository, categoryRepository, assetRepository, productAttributeValueRepository }) => {
    return async (event) => {
        const { id } = event.pathParameters;
        const body = event.body;
        ...
        try {
            ...
            if (productData.code || categoryId) {
                ...
                if (categoryId) {
                    ...
                    if (!existing.category) {
                        const category = await categoryRepository.getCategory(existing.categoryId);
                        if (!category)
                            throw new createError.NotFound("Existing Category not found");
                        targetCategoryCode = category.code;
                    }
                    else {
                        targetCategoryCode = existing.category.code;
            ...
            if (!targetCategory)
                throw new createError.NotFound("Category not found");
            const allowedAttributeValueIds = targetCategory.allowedAttributeValueIds;
            ...
            if (assetType && assetKey && mimeType) {
                ...
                updated = await productRepository.getProduct(id);
            }
            return apiResponseDTO({
                statusCode: 200,
            ...
            if (err instanceof Prisma.PrismaClientKnownRequestError) {
                if (err.code === "P2002") {
                    ...
                    if (targets.includes("slug"))
                        throw new createError.Conflict("Product slug already exists");
                    throw new createError.Conflict("Unique constraint failed");
                }
                if (err.code === "P2025")
                    throw new createError.NotFound("Product or category not found");
            }
            throw new createError.InternalServerError("Failed to update product");
        }
    };
