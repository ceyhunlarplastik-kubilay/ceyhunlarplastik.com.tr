...
export const createProductAttributeValueHandler = ({ productAttributeValueRepository, productAttributeRepository, assetRepository, }) => {
    return async (event) => {
        ...
        try {
            ...
            if (!attribute)
                throw new createError.NotFound("Attribute not found");
            let validatedParentValueId = null;
            if (parentValueId) {
                const parentValue = await productAttributeValueRepository.getValueById(parentValueId);
                if (!parentValue)
                    throw new createError.NotFound("Parent value not found");
                ...
                if (attribute.code !== ATTRIBUTE_CODES.productionGroup &&
