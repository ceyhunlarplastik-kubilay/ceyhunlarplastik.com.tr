...
export const updateUserSupplierHandler = ({ userRepository, supplierRepository, customerRepository }) => async (event) => {
    ...
    const { supplierId, customerId, assignedSupplierIds, assignedCustomerIds, } = event.body ?? {};
    ...
    if (assignedSupplierIds?.length) {
        const suppliers = await Promise.all(assignedSupplierIds.map((supplierIdItem) => supplierRepository.getSupplier(supplierIdItem)));
    ...
    if (assignedCustomerIds?.length) {
        const customers = await Promise.all(assignedCustomerIds.map((customerIdItem) => customerRepository.getCustomer(customerIdItem)));
        if (customers.some((customer) => !customer)) {
            throw new createError.NotFound("One or more assigned customers were not found");
        }
    }
    const updated = await userRepository.updateAssignments(user.id, user.groups, {
