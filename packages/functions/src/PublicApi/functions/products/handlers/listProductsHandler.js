...
export const listProductsHandler = ({ productRepository }) => async (event) => {
    const query = event.queryStringParameters ?? {};
    /* const attributeFilters = Object.entries(query).filter(
        ([key]) =>
            !["page", "limit", "search", "sort", "order", "categoryId"].includes(key)
    ); */
    const attributeValueIds = typeof query.attributeValueIds === "string"
    ...
    const attributeFilters = Object.entries(query).filter(([key]) => ![
        "page",
        "limit",
        "search",
        "sort",
