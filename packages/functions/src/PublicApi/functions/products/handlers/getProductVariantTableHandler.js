...
const sortMeasurements = (measurements) => [...measurements].sort((left, right) => {
    const displayOrderDiff = (left.measurementType.displayOrder ?? Number.MAX_SAFE_INTEGER) -
...
export const getProductVariantTableHandler = ({ productVariantRepository }) => {
    return async (event) => {
        const productId = event.pathParameters?.id;
        if (!productId)
            throw new createError.BadRequest("productId required");
        ...
        try {
            ...
            if (search) {
                ...
                deduped = deduped.filter(v => v.versionCode.toLowerCase().includes(s) ||
            ...
            const dynamicColumns = Array.from(new Set(paginated.flatMap(v => v.measurements.map((m) => m.measurementType.code))));
