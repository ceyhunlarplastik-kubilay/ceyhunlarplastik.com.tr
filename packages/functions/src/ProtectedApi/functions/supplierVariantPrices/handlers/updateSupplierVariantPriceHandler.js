...
export const updateSupplierVariantPriceHandler = ({ productVariantSupplierRepository }) => async (event) => {
    const user = event.user;
    if (!user) {
        throw new createError.Forbidden("User context missing");
    }
    const { id } = event.pathParameters;
    const { price, operationalCostRate, netCost, profitRate, listPrice, paymentTermDays, supplierVariantCode, supplierNote, minOrderQty, stockQty, currency, } = event.body;
    ...
    const updated = await productVariantSupplierRepository.updateProductVariantSupplier(id, buildApprovedVariantPricingUpdate(existing, {
        price,
        operationalCostRate,
        netCost,
        profitRate,
