...
import { assertCustomerManagementAccess, assertCustomerPortalAccess, assertSupplierManagementAccess, } from "@/core/helpers/crm/access";
import { buildCustomerUpdateData } from "@/core/helpers/crm/customerUpdateData";
const CUSTOMER_SORT_FIELDS = ["fullName", "companyName", "email", "createdAt"];
const SUPPLIER_SORT_FIELDS = ["name", "createdAt"];
function mapFeaturedProducts(data) {
    return data.map((item) => ({
        ...item,
        product: mapProductWithAssets(item.product),
    }));
}
export const listManagedCustomersHandler = ({ customerRepository }) => {
    return async (event) => {
...
export const getManagedCustomerHandler = ({ customerRepository }) => {
    return async (event) => {
...
export const updateManagedCustomerHandler = ({ customerRepository, productAttributeValueRepository, }) => {
    return async (event) => {
...
export const convertManagedCustomerHandler = ({ customerRepository }) => {
    return async (event) => {
        const requester = event.user;
        if (!requester)
            throw new createError.Unauthorized("Authentication required");
...
export const listManagedCustomerFeaturedProductsHandler = ({ customerRepository }) => {
    return async (event) => {
        const customer = await customerRepository.getCustomer(event.pathParameters.id);
        if (!customer)
            throw new createError.NotFound("Customer not found");
...
export const replaceManagedCustomerFeaturedProductsHandler = ({ customerRepository, productRepository, }) => {
    return async (event) => {
...
export const listManagedCustomerAssignedProductsHandler = ({ customerRepository }) => {
    return async (event) => {
        const customer = await customerRepository.getCustomer(event.pathParameters.id);
        if (!customer)
            throw new createError.NotFound("Customer not found");
...
export const replaceManagedCustomerAssignedProductsHandler = ({ customerRepository, productRepository, }) => {
    return async (event) => {
        if (!productRepository) {
            throw new createError.InternalServerError("Product repository not configured");
        }
...
export const listManagedCustomerVisitsHandler = ({ customerRepository }) => {
    return async (event) => {
        const customer = await customerRepository.getCustomer(event.pathParameters.id);
        if (!customer)
            throw new createError.NotFound("Customer not found");
...
export const createManagedCustomerVisitHandler = ({ customerRepository }) => {
    return async (event) => {
        const requester = event.user;
        if (!requester)
            throw new createError.Unauthorized("Authentication required");
...
export const updateManagedCustomerVisitHandler = ({ customerRepository }) => {
    return async (event) => {
        const customer = await customerRepository.getCustomer(event.pathParameters.id);
        if (!customer)
            throw new createError.NotFound("Customer not found");
...
export const deleteManagedCustomerVisitHandler = ({ customerRepository }) => {
    return async (event) => {
        const customer = await customerRepository.getCustomer(event.pathParameters.id);
        if (!customer)
            throw new createError.NotFound("Customer not found");
...
export const listManagedSuppliersHandler = ({ supplierRepository }) => {
    return async (event) => {
        const requester = event.user;
        if (!requester)
            throw new createError.Unauthorized("Authentication required");
...
export const getManagedSupplierHandler = ({ supplierRepository }) => {
    return async (event) => {
...
export const getPortalCustomerHandler = ({ customerRepository }) => {
    return async (event) => {
...
export const getPortalCustomerFeaturedProductsHandler = ({ customerRepository }) => {
    return async (event) => {
        const customerId = event.user?.customerId;
        if (!customerId)
            throw new createError.Forbidden("Customer portal access denied");
...
export const getPortalCustomerAssignedProductsHandler = ({ customerRepository }) => {
    return async (event) => {
        const customerId = event.user?.customerId;
        if (!customerId)
            throw new createError.Forbidden("Customer portal access denied");
