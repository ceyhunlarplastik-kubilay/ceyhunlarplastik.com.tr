...
/**
 * shadcn kaynağından İKİ BİLİNÇLİ SAPMA var — `npx shadcn add … --overwrite`
 * bu dosyayı geri alırsa ikisi de kaybolur ve testler düşer:
 *
 *  1. Fiziksel yön yardımcıları mantıksal karşılıklarıyla değiştirildi
 *     (inset/kenarlık/dolgu/hizalama; `start`, `end`, `-s`, `-e`, `pe`,
 *     `text-start`). Repo Arapça yayınlıyor ve
 *     `components/logicalProperties.test.ts` bu klasörü kapıda tutuyor —
 *     kural yalnız RTL'de görünür, LTR render'ı birebir aynıdır.
 *  2. `SidebarMenuSkeleton` genişliği `Math.random()` yerine `useId`'den
 *     türetiliyor (render saflığı + hydration uyuşmazlığı).
 */
...
function SidebarMenuSkeleton({
  ...
  // shadcn'in ürettiği hâlinden SAPMA (bilerek): orijinali genişliği
  // `Math.random()` ile üretiyordu. Bu hem render sırasında saf olmayan bir
  // çağrı (lint kuralı), hem de sunucu ile istemcinin FARKLI genişlik üretmesi
  // demekti — iskelet her yüklemede hydration uyuşmazlığı riski taşıyordu.
  // Genişlik artık `useId`'den türeyen deterministik bir değer: aynı görsel
  // çeşitlilik, sunucu ve istemcide aynı sonuç.
