export const USER_PHONE_INPUT_REGEX = /^[+\d\s()-]{8,24}$/

export function normalizeUserPhoneInput(input: string): string | null {
    const trimmed = input.trim()
    if (!trimmed || !USER_PHONE_INPUT_REGEX.test(trimmed)) {
        return null
    }
