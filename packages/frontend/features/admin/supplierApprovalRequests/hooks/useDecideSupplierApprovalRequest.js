    ...
    return useMutation({
        ...
        onSuccess: (data) => {
            ...
            if (data.approvalRequest) {
                queryClient.setQueriesData({ queryKey: ["admin-supplier-approval-requests"] }, (current) => {
                    if (!current)
                        return current;
                    return {
                        ...current,
                        data: current.data.map((request) => request.id === data.approvalRequest?.id ? data.approvalRequest : request),
                    };
                });
            }
