...
export function buildRequestItemPayload(items: PortalRequestDraftItem[]): NonNullable<PortalBusinessRequestInput["items"]> {
    return items.map((item) => ({
        ...
        data: {
            ...
            generalDiscountPercent: item.generalDiscountPercent ?? null,
            priceSource: item.priceSource ?? null,
            specialPriceId: item.specialPriceId ?? null,
            specialPriceEligible: item.specialPriceEligible ?? null,
            specialPriceIneligibilityReason: item.specialPriceIneligibilityReason ?? null,
            specialPriceIneligibilityMessage: item.specialPriceIneligibilityMessage ?? null,
