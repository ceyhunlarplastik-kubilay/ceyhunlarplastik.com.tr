...
export async function generateCategoryAssetUpload({ categorySlug, assetRole, fileName, contentType, }) {
...
export async function generateProductAssetUpload({ productSlug, assetRole, fileName, contentType, }) {
    const safeName = sanitizeFileName(fileName);
    const ext = safeName.includes(".") ? safeName.split(".").pop() : undefined;
    const uuid = randomUUID();
    const folder = getFolderByRole(assetRole);
...
export async function generateProductAttributeValueAssetUpload({ attributeCode, valueSlug, assetRole, fileName, contentType, }) {
    const safeName = sanitizeFileName(fileName);
    const ext = safeName.includes(".") ? safeName.split(".").pop() : undefined;
    const uuid = randomUUID();
    const folder = getFolderByRole(assetRole);
