...
import { ALL_USER_GROUPS, BUSINESS_USER_GROUPS, } from "@/core/helpers/userAccess/types";
function normalizeNextAccessStatus(group, accessStatus) {
    if (accessStatus)
        return accessStatus;
    return group === "user" ? "PENDING_REVIEW" : "ACTIVE";
}
function assertRequesterCanAssignGroup(requester, nextGroup) {
    if (requester.isOwner)
        return;
    if (!requester.isAdmin) {
        throw createError.Forbidden("Only admin/owner can change user roles");
    }
    if (!BUSINESS_USER_GROUPS.includes(nextGroup)) {
        throw createError.Forbidden("Admin cannot assign privileged roles");
    }
}
export async function updateUserAccess(input) {
    const { requester, targetUserId, nextGroup, supplierId, customerId, reason, accessStatus, deps, } = input;
    ...
    for (const group of toRemove) {
        if (user.groups.includes(group)) {
            await deps.cognitoRepository.removeFromGroup(deps.userPoolId, user.cognitoSub, group);
        }
    }
    if (!user.groups.includes(nextGroup)) {
        await deps.cognitoRepository.addToGroup(deps.userPoolId, user.cognitoSub, nextGroup);
    }
