...
export const productRepository = () => {
    ...
    const listProducts = async (query) => {
        ...
        const buildAttributeWhere = (attrCode, rawValue) => {
        ...
        const naturalCodeCompare = (a, b) => a.localeCompare(b, "tr", {
            numeric: true,
            sensitivity: "base",
        });
        ...
        if (sortByCode) {
            ...
            if (pagedIds.length === 0) {
                ...
                data = pagedData.sort((left, right) => (orderMap.get(left.id) ?? Number.MAX_SAFE_INTEGER) -
    ...
    const getProduct = (id) => prisma.product.findUniqueOrThrow({
        where: { id },
        include: baseInclude
    });
    const getProductBySlug = (slug) => prisma.product.findUniqueOrThrow({
        where: { slug },
        include: baseInclude
    });
