...
function enrichHierarchyAttributeValues(attributeValues) {
    const result = [];
    const seen = new Set();
    const pushValue = (value) => {
        if (!value?.id || !value?.attribute?.code)
            return;
...
export function mapProductWithAssets(product) {
    ...
    const animation = assets.find((a) => a.role === AssetRole.ANIMATION);
    const gallery = assets.filter((a) => a.role === AssetRole.GALLERY);
    const documents = assets.filter((a) => a.role === AssetRole.DOCUMENT);
    const technicalDrawings = assets.filter((a) => a.role === AssetRole.TECHNICAL_DRAWING);
    return {
        id: product.id,
        code: product.code,
        name: product.name,
