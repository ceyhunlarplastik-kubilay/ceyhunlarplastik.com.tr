...
export function resolveProductVariantSupplierPricing(input, existing = {}) {
    const result = {};
    ...
    if (hasPrice)
        result.price = roundTo(input.price, 2);
    if (hasOperationalRate)
        result.operationalCostRate = roundTo(input.operationalCostRate, 2);
    if (hasNetCost)
        result.netCost = roundTo(input.netCost, 2);
    if (hasProfitRate)
        result.profitRate = roundTo(input.profitRate, 2);
    if (hasListPrice)
        result.listPrice = roundTo(input.listPrice, 2);
    const resolvedNetCost = hasNetCost
        ? input.netCost
        : hasPrice
