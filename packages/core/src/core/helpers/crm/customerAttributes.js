...
export async function validateCustomerAttributeSelection(productAttributeValueRepository, input) {
    ...
    const usageAreaValues = await Promise.all(usageAreaIds.map((id) => productAttributeValueRepository.getValueById(id)));
    for (const value of usageAreaValues) {
        if (!value)
            throw new createError.BadRequest("One or more usage_area values are invalid");
