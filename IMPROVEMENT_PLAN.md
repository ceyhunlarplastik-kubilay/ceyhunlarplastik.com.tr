...
**P0.2 — CORS'u stage-aware daralt** · kapsam: küçük
...
- Etki: **backend** (middy.ts — 197 handler'ın tamamı) + **frontend** (davranış değişmemeli; `lib/http/client.ts` istekleri aynı origin listesinde kalmalı) + **infra** (izinli origin'lerin stage domain'lerinden türetilmesi). Dev stage'lerde localhost origin'i unutulmamalı, yoksa lokal geliştirme kırılır.
