...
### Asenkron asset yükleme (S3 event-driven) — pilot: kategori · Dilim 1 ✅ (2026-09-04, LOG) *(kullanıcı talebiyle)*
- **Amaç:** kategori asset yüklemesinde kullanıcı yalnız S3 PUT'unu beklesin; DB kaydı + refetch arka planda. Desen: presign `PENDING_UPLOAD` satırı yaratır → S3 PUT → `s3:ObjectCreated:*` (`filterPrefix: "categories/"`) → **doğrudan Lambda** satırı `ACTIVE`'e çevirir. Taşıma kararı onaylı: EventBridge değil `bucket.notify`. Zombi cron Dilim 4.
- **Dilim 2 (backend + infra):** `createCategoryAssetUploadHandler` `assetRepository` dep alır, body'ye `categoryId`(uuid)+`assetType` ekler → `createPendingAsset` + id'li key (`categories/<slug>/<rol>/<assetId>.<ext>`) + dönüşe `assetId`; `expiresIn` 60→900s; `createCategoryAssetUploadValidator`'a yeni alanlar. **YENİ** `infra/assetLifecycle.ts`: `publicBucket.notify({ events: ["s3:ObjectCreated:*"], filterPrefix: "categories/" })` → **YENİ** `packages/functions/src/AssetLifecycle/functions/confirmCategoryAssetUpload.ts` (VPC+rds link, Powertools, idempotent `confirmUploadedAsset`, PRIMARY demote-on-confirm). `sst.config.ts` → `await import("./infra/assetLifecycle")`. Bucket paylaşımsız (stage adı bucket adında) ama kod prod'a gider → `sst diff --stage prod` onaylı.
- **Dilim 3 (frontend UX):** `features/admin/categories/components/asset/AssetUploader.tsx` bloklayan `updateCategory`+`refetchCategory`'yi bırakır → optimistic pending asset + `toast` + **YENİ** `usePendingAssetReconciler` poll. `AssetGrid`/`AssetPreviewPanel` `uploadStatus==="PENDING_UPLOAD"` için `Badge`+`Loader2`, sil/önizleme kısıtlı. **Public/liste okumalarına `uploadStatus=ACTIVE` filtresi** — `categoryRepository` `categoryInclude` + tüm `category.assets` tüketicileri (admin + public kategori handler'ları `mapCategoryWithAssets` paylaşıyor).
- **Dilim 4 (opsiyonel):** günlük `sst.aws.Cron` zombi-sweep (`PENDING_UPLOAD` + `createdAt < now()-24h` sil, `googleMaps.ts` cron deseni). İstenirse ürün/materyal/attribute-value asset yüklemelerine aynı desen; 2. tüketici (thumbnail/tarama) gerekirse EventBridge Bus'a taşı.
- Etki: **core** (schema ✅ / repo ✅ / mapper ✅) · **functions** (presign handler + yeni `AssetLifecycle` Lambda + validator) · **infra** (`assetLifecycle.ts`, `sst.config.ts`) · **frontend** (`features/admin/categories/*`, `features/public/assets/types.ts` ✅).
