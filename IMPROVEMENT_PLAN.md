...
- **⚠️ DEPLOY EDİLMEDİ:** kullanıcı talebiyle hiçbir stage'e deploy/diff alınmadı. Deploy öncesi: `sst diff --stage prod` → `MyPostgres` yalnız `deletionProtection`/`skipFinalSnapshot`/`finalSnapshotIdentifier` **update-in-place** göstermeli (REPLACE değil, başka kaynak değil). **Açık iş kalanı:** RPO/RTO onayı + ilk restore drill'i (gerçek RTO'yu öğren).
