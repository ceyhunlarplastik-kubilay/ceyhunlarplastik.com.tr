...
### Tedarikçi sözlüğü teknik resmi (async yükleme) — Dilim 1–2 ✅ (2026-09-04, LOG) *(kullanıcı talebiyle)*
...
- **Yapıldı (Dilim 1–2, LOG):** `Asset.productSupplierCodeId` FK + `ProductSupplierCode.assets`; `generateProductSupplierCodeAssetUpload` presign helper (key `product-supplier-codes/{productId}/{codeId}/{assetId}.{ext}`, 900s); `productSupplierCodeRepository` `list/create/update` yanıtına `technicalDrawing` + `findForProduct` yetki kontrolü + `remove()` S3 temizliği; `POST /products/{id}/supplier-codes/{codeId}/technical-drawing/presign` ucu (`createProductSupplierCodeAssetUploadHandler`, type/role sabit TECHNICAL_DRAWING, request+response validator); `confirmProductSupplierCodeAssetUpload.ts` Lambda (prefix guard `product-supplier-codes/`, sadece PENDING→ACTIVE); `infra/assetLifecycle.ts` 2. kardeş notification. Kubi runtime testi + `sst diff --stage prod` önizleme kullanıcıda.
