...
### Asenkron asset yükleme (S3 event-driven) — pilot: kategori · Dilim 1–2 ✅ (2026-09-04, LOG) *(kullanıcı talebiyle)*
- **Yapıldı (LOG):** şema `AssetUploadStatus` + `Asset.uploadStatus`/`uploadedAt`; presign artık `PENDING_UPLOAD` satırı yazıyor (id'li key, `expiresIn` 900s); `infra/assetLifecycle.ts` `publicBucket.notify` → `confirmCategoryAssetUpload` Lambda satırı `ACTIVE`'e çeviriyor (idempotent, PRIMARY demote-on-confirm); `AssetUploader` bloklayan `updateCategory` çağrısını bıraktı. Kubi'de `sst diff --stage prod` + uçtan uca test kullanıcıda.
- **Dilim 3 (frontend UX + public filtre):** `AssetUploader.tsx` kalan `refetchCategory()` beklemesini de bırakır → optimistic pending asset + **YENİ** `usePendingAssetReconciler` poll. `AssetGrid`/`AssetPreviewPanel` `uploadStatus === "PENDING_UPLOAD"` için `Badge`+`Loader2`, sil/önizleme kısıtlı. **Public/liste okumalarına `uploadStatus = ACTIVE` filtresi** — `categoryRepository.categoryInclude` (`assets: true` şu an TÜM asset'leri çekiyor) + admin/public `mapCategoryWithAssets` tüketicileri; yönetim dialog'u PENDING'i rozetle gösterir, gerisi ACTIVE.
...
- Etki: **frontend** (`features/admin/categories/*`) · **core** (`categoryRepository`) · **functions** (kategori handler'ları / mapper filtre parametresi).
